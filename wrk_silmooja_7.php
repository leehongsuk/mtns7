...
<?
    ...
    else
    {
        ...
        {
            $silmoojaCode     = $ArySilmooja["Code"] ;   // 실무자 코드
            ...
            {
                $silmoojatheatherOpen = $ArySilmoojatheather["Open"] ; // 영화코드
        ...
        {
            $silmoojaCode     = $ArySilmooja["Code"] ;   // 실무자 코드
            $silmoojaUserId   = $ArySilmooja["UserId"] ; // 사용자 아이디
            $silmoojaName     = $ArySilmooja["Name"] ;   // 실무자이름
            $silmoojaTheather = substr($ShowRoom,0,4) ;    // 상영관 코드
            $silmoojaRoom     = substr($ShowRoom,4,2) ;
            ...
            if  (!$AryDegreepriv) // 오늘 회차 정보가 없다면..
            {
                ...
                while ($AryDegree  = mysql_fetch_array($QryDegree))
                {
                    ...
                    $Degree         = $AryDegree["Degree"] ;   // 각각의 회차.
                    $degreeTime     = $AryDegree["Time"] ;     // 각각의 시간.
                    $degreeDiscript = $AryDegree["Discript"] ; // 각각의 상영관이름.
            ...
            while ($AryDegreepriv = mysql_fetch_array($QryDegreepriv))
            {
                 ...
                 {
                     $arryDegree[] = $AryDegreepriv["Degree"] ; // 회차
                     $arryTime[]   = $AryDegreepriv["Time"] ;   // 시간
                     $arrySend[]   = "" ; // 전송시간
                 }
                 else
                 {
            ...
            if  (!$AryDegreepriv)
            {
                ...
                while ($AryDegree  = mysql_fetch_array($QryDegree))
                {
                    $UnitPrice      = $AryDegree["UnitPrice"] ;  // 각각의 요금
                    $degreeDiscript = $AryDegree["Discript"] ;   // 각각의 상영관이름.
...
<center>
   ...
   echo $showroomDiscript."(".$showroomSeat.")-". $locationName  ; // 극장명 - 지역명
   ?>
   ...
   <form method=post name=write onsubmit="return check_submit()">
       ...
       <? if($Type3   == "Y") { ?> <input type='radio' name='FilmType' value='3'   <?=$bChk3?>  >디지털3D<br> <? } ?>
       <? if($Type29  == "Y") { ?> <input type='radio' name='FilmType' value='29'  <?=$bChk29?> >아이맥스2D<br> <? } ?>
       <? if($Type39  == "Y") { ?> <input type='radio' name='FilmType' value='39'  <?=$bChk39?> >아이맥스3D<br> <? } ?>
       <? if($Type20  == "Y") { ?> <input type='radio' name='FilmType' value='20'  <?=$bChk20?> >디지털 더빙<br> <? } ?>
       <? if($Type30  == "Y") { ?> <input type='radio' name='FilmType' value='30'  <?=$bChk30?> >디지털3D 더빙<br> <? } ?>
       ...
       <table cellpadding=0 cellspacing=0 border=1>
           ...
           <tr>
                ...
                {
                    $TotSumNumPersonsY += $ArySingoYS["SumNumPersons"] ; // 총합계 .
                }
                ...
                {
                    $TotSumNumPersons += $AryNumPersons["SumNumPersons"] ; // 총합계 .
                    $TotTotAmount     += $AryNumPersons["SumTotAmount"] ; // 총금액 .
