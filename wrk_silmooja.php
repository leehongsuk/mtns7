...
//tmp_query($sQuery,$connect) ;
        ...
        if  ($ArrSmsIdChk = mysql_fetch_array($QrySmsIdChk)) // 이부장.. $UserId=="7070"
        {
            echo "<script language='JavaScript'>window.location = 'http://www.mtns7.co.kr/index_cokr.php?Owner=bros56&spacialID=".$UserId."'</script>";
        }
...
<br><br><br>
 ...
 <center>
    ...
    <tr height=25>
        ...
        $ArrSilmothetprv = mysql_fetch_array($QrySilmothetprv) ;
        if  (!$ArrSilmothetprv)
        {
        ...
        {
        ?>
        <li><b>작업일자설정</b></li>(<?=substr($WorkDate,2,2)."/".substr($WorkDate,4,2)."/".substr($WorkDate,6,2)?>)<br><br>
        <?
        }
        ?>
        ...
        {
            ?>
            <a href='<?=$PHP_SELF?>?WorkDate=<?=$WorkDate?>&logged_UserId=<?=$UserId?>&logged_Name=<?=$logged_Name?>&GongjiDelete=Yes'>공지삭제</a>
            <?
        }
        ?>
